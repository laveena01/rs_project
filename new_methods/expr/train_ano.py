...
def train_model(model, optimizer, scheduler, dataloaders, dataset_sizes,
    ...
    for epoch in range(num_epochs):
        ...
        # if epoch == 61:
        #     optimizer.param_groups[0]['lr'] = 0.0000
        #     optimizer.param_groups[1]['lr'] = 0.0000
        #     optimizer.param_groups[2]['lr'] = 0.001
        #     optimizer.param_groups[3]['lr'] = 0.002
...
def get_optim(model):
    ...
    optimizer = optim.SGD([{'params': weight_list, 'lr': lr},
                           ...
                           {'params': added_weight_list, 'lr': lr},
                           {'params': added_bias_list, 'lr': lr * 2}],
        ...
        for cos in COS:
            for k in K:
                ...
                if select_model == 'CAM':
                    ...
                    from new_methods.model.resnet_ACoL import model
                    model_ft = model(pretrained=True, num_classes=len(categories))
                    ...
                    model_ft = model(pretrained=True, num_classes=len(categories), cos_alpha=cos, num_maps=k, pam=pam, cam=cam)
                ...
                # train model
                # model_path = path + select_model +str(i)+ '_new.pkl'
                # print model_ft
                # model_path = path + select_model + '_'+str(cos)+'_'+ str(k) +'_0'+'_none.pkl'
                model_path = path + select_model + '_' + str(cos) + '_' + str(k) + '_0_' + method + '.pkl'
                train_model(model_ft, optimizer_ft, exp_lr_scheduler, dataloaders,
                    dataset_sizes, model_path, num_epochs=3)
