...
from data.airplane_data import *
from CAM_tools import *
import os
from PIL import Image
from utils.visualise import show
...
def test(model, store_path):
    ...
    test_data_size = 0
    corrects = 0.0
    str = r'/home/xxxxujian/SourceCode/data/test_airplane'
    ...
    for img in os.listdir(str + '/img'):
        ...
        bbox_gt = generateBBoxGT(str + '/annotation/' + os.path.splitext(img)[0] + '.xml')
...
if __name__ =="__main__":
    store_path = '/home/xxxxujian/SourceCode/X_Project/Utils/no_DA/'
    path = 'model_trained/params_single2_'
    select_model = 'DA_PAM'
    methods = ['pam', 'cam']
    K = [4, 8, 16, 32]
    COS = [0.01, 0.02, 0.1, 0.5, 0.7]
                ...
                for cos in COS:
                    ...
                    for i in [2]:
                        spamwriter.writerow([str(method), str(k), str(cos), res[1][i], res[2][i], res[3][i],
                                             res[1][i] / (res[1][i] + res[2][i]), res[1][i] / (res[1][i] + res[3][i]), res[0]])
                    # count = count + 1
                    # CA.append(count)
                    # PA.append(res[0])
                    # TA.append(res[1])
