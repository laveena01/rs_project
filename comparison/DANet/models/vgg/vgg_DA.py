...
class VGG(nn.Module):
    def __init__(self, features, num_classes=1000, args=None):
        ...
        self.cos_alpha = 0.2#args.cos_alpha
        self.num_maps = 8#int(args.num_maps)
        # added layer
    ...
    def forward(self, x):
        ...
        rootResult_multimaps = self.cls3(rootResult)
        rootResult_multimaps = rootResult_multimaps.view(batch_size, 11, self.num_maps, 28, 28)
...
if __name__ == '__main__':
    ...
    output = model(x)
    model.get_loss(output, torch.randn(1, 1), torch.randn(1, 1), torch.randn(1, 1))
